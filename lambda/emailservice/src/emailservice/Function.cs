...
namespace emailservice
{
    public class Function
    {
        ...
        public string FunctionHandler(Requests input, ILambdaContext context)
        {
            ...
            if (string.IsNullOrEmpty(currentOrg.Name) || string.IsNullOrEmpty(currentContact.GUID))
            {
                Console.WriteLine($"Something went wrong while fetching Org with id: {input.OrgId} and Contact with email: {input.Email}");
                Environment.Exit(Environment.ExitCode);
            }

            string[] recips = { "benjamin.elser18@gmail.com", "belser@elsersmusings.com", "1228883b@gytf.edu" };
            ...
            $"<p>{currentOrg.Id}</p>";
            EmailService es = new EmailService("belser@elsersmusings.com","Ben Elser", recips, "NEW BLOG REQUEST", BODY);
